...
public class AzureSpatialAnchorsScript : MonoBehaviour
{
    ...
    public GameObject ros_publisher;
    public GameObject cursor;

    public List<GameObject> getAnchorList()
    {

        return _foundOrCreatedAnchorGameObjects;
    }
    ...
    public async void EyeGazeCreateAnchor()
    {
        Color original_color = cursor.GetComponent<MeshRenderer>().material.color;
        cursor.GetComponent<MeshRenderer>().material.color = Color.red;
        var position = CoreServices.InputSystem.EyeGazeProvider.HitPosition;
        await _spatialAnchorManager.StartSessionAsync();
        if (!IsAnchorNearby(position, out GameObject anchorGameObject))
            await CreateAnchor(position);
        cursor.GetComponent<MeshRenderer>().material.color = original_color;
    }

    public async void EyeGazeDeleteAnchor()
    {
        var position = CoreServices.InputSystem.EyeGazeProvider.HitPosition;
        await _spatialAnchorManager.StartSessionAsync();
        if (IsAnchorNearby(position, out GameObject anchorGameObject))
    ...
    private async Task CreateAnchor(Vector3 position)
    {
        ...
        cloudSpatialAnchor.Expiration = DateTimeOffset.Now.AddDays(60);
        ...
        try
        {
            ...
            ///new
            ros_publisher.GetComponent<RosPublisherScript>().SendAnchorID(cloudSpatialAnchor.Identifier);
            // ros_publisher.GetComponent<RosPublisherExample>().is_go = false;
            ///end new
